...
errfile_path = os.path.expanduser("galp.err")
errfile = open(errfile_path, 'w')
os.dup2(errfile.fileno(), sys.stderr.fileno())

class GalaInput:
    """
    A class to handle the initialization and configuration of GALP inputs based on the
    parameters specified in the GALP.inp file and optionally overridden by the CONTROL file.
    """

    def __init__(self, working_directory: str) -> None:
        """
        Initializes the GalaInput class with the provided directory path for GALP input files.

        Args:
            working_directory (str): The directory path containing GALP input files.
        """
        with open(f"{working_directory}/GALP.inp", "r") as f:
            lines = f.readlines()
        ...
        # General Input
        self.method = self._get_method(lines[22])
        self.directory = self._get_directory(lines[24])
        self.max_number_bs = self._get_max_number_bs(lines[26])
        self.max_bs_energy = self._get_max_bs_energy(lines[28])
        self.cif_of_maxima = lines[30].strip("\n").upper() == "T"
        self.output_directory = self._get_output_directory()

        # Binding Site
        self.bs_algorithm = self._get_bs_algorithm(lines[34])
        self.overlap_tol = self._get_ov_tol(lines[37])
        self.rmsd_cutoff = self._get_rmsd_cutoff(lines[39])
        self.include_h = lines[41].strip("\n").upper() == "T"

        # MD Parameters
        self.md_exe = lines[45].strip("\n")
        self.opt_binding_sites = lines[47].strip("\n").upper() == "T"
        self.opt_steps = self._get_opt_steps(lines[49])
        self.timestep = self._get_timestep(lines[51])
        self.md_cutoff = self._get_md_cutoff(lines[53])
        self.md_cpu = self._get_md_cpu(lines[55])

        # GCMC Analysis Parameters
        self.gcmc_sigma = self._get_sigma(lines[59])
        self.gcmc_radius = self._get_radius(lines[61])
        self.gcmc_cutoff = self._get_cutoff(lines[63])
        self.gcmc_write_folded = lines[65].strip("\n").upper() == "T"
        self.gcmc_grid_factor = self._get_gcmc_grid_factor(lines[67])
        self.gcmc_write_smoothed = lines[69].strip("\n").upper() == "T"

        # Override Parameters from CONTROL/simulation.json file
        if self.method == "FASTMC" and os.path.isfile(os.path.join(self.directory, "CONTROL")):
        ...
        if self.method == "RASPA" and os.path.isfile(os.path.join(self.directory, "simulation.json")):
            json_file = os.path.join(self.directory, "simulation.json")
            try:
                grid = self._get_json_option(json_file, ["Systems", 0, "NumberOfUnitCells"])
                if (
                    isinstance(grid, (list, tuple))
                    and len(grid) == 3
                    and all(isinstance(x, int) for x in grid)
                ):
                    self.gcmc_grid_factor = tuple(grid)
                    self._log_temp(
                        "INFO",
                        f"Updated Grid Factors from simulation.json. Using: {self.gcmc_grid_factor}",
                    )
            ...
            try:
                ...
                if isinstance(components, list):
                    ...
                    if names:
                        self.raspa_guests = names
                        self._log_temp(
                            "INFO",
                            f"Updated guests from simulation.json. Using: {self.raspa_guests}",
                        )
            except (KeyError, TypeError):
                pass

        # Probability Plot Guest and Site Selection (deprecated)
        # Implemented then never used, instead defaulted to ALL
        # For simplicity, we keep it but default to ALL
        # self.selected_guest = self._get_selected_guest(lines[84])
        # self.selected_site = self._get_selected_site(lines[86])
        self.selected_guest = "ALL"
        self.selected_site = "ALL"

        # ML Probability plot generation
        self.gen_ml_plot = lines[73].strip("\n").upper() == "T"
        if self.gen_ml_plot:
            self._log_temp("INFO", "ML Probability plot generation selected.")
    ...
    def _get_output_directory(self) -> str:
        ...
        output_directory = os.path.join(self.directory, "GALP_Output")
    ...
    def _get_json_option(self, json_file: str, keys: list):
        """Reads a JSON file and retrieves the value associated with the specified key."""
        ...
        for key in keys:
            if isinstance(cur, dict):
                ...
                raise TypeError(f"Cannot descend into type {type(cur)} at step {key}")
        return cur
...
class GuestStructure:
    ...
    def __init__(self, gala: GalaInput):
        """
        Initialize the GuestStructure class with the specified method, directory, and GALP instance.
        ...
        if self.method == "RASPA":
            (
                ...
                site_data,
                information_as_dictionary,
            ), guest_reduced = self.parse_raspa()
            self.guest_molecules = []
    ...
    def parse_raspa(
        ...
        Tuple[
            List[List[str]],                         # valid_labels
            List[List[Tuple[float, float, float]]],  # coordinates
            List[List[str]],                         # species (pseudo atoms)
            List[List],                              # site_data_unfiltered
            List[List],                              # site_data_grouped
            List[Dict[str, List]],                   # full guest dicts
        ],
        List[str],                                   # guests_reduced
    ]:
        
        def _find_guest_json_file(directory: str, guest_name: str) -> str:
            path = os.path.join(directory, f"{guest_name}.json")
            if not os.path.isfile(path):
                raise FileNotFoundError(f"Cannot find guest json file: {path}")
            return path

        def _is_lj_type(t):
            t = str(t).strip().lower()
            return t in {"lennard-jones", "lennard_jones", "lennardjones"}

        def build_lj_params(force_field):
            lj = {}
            for entry in force_field.get("SelfInteractions", []):
                if not _is_lj_type(entry.get("type", "")):
                    continue
                params = entry.get("parameters", [])
                if len(params) < 2:
                    continue
                eps_K = float(params[0])
                sigma = float(params[1])

                lj[str(entry["name"]).strip()] = (
                    self._epsilon_K_to_kcal(eps_K),
                    sigma,
                )
            return lj
        ...
        self._structure_name = "framework"

        ff_path = os.path.join(directory, "force_field.json")
        if not os.path.isfile(ff_path):
            raise FileNotFoundError(f"Cannot find force_field.json in {directory}")

        with open(ff_path, "r") as f:
            force_field = json.load(f)

        pseudo_atoms_meta = {}
        for entry in force_field.get("PseudoAtoms", []):
            if "name" in entry:
                pseudo_atoms_meta[entry["name"]] = entry

        lj_params = build_lj_params(force_field)
        ...
        for guest_name in self.gala.raspa_guests:

            guest_file = _find_guest_json_file(directory, guest_name)
            with open(guest_file, "r") as f:
                guest_json = json.load(f)

            if "pseudoAtoms" not in guest_json:
                raise ValueError(f"{guest_file} missing 'pseudoAtoms'")

            guest = reset_guest()

            for pseudo_name, coord in guest_json["pseudoAtoms"]:

                if pseudo_name not in pseudo_atoms_meta:
                    raise KeyError(
                        f"Pseudo atom '{pseudo_name}' used in {guest_file} "
                        f"but not defined in force_field.json"
                    )

                meta = pseudo_atoms_meta[pseudo_name]

                element = meta["element"]
                mass = float(meta.get("mass"))
                charge = float(meta.get("charge", 0.0))
                lj = lj_params.get(pseudo_name, None)

                coord = (float(coord[0]), float(coord[1]), float(coord[2]))

                guest["valid_labels"].append(element)
                guest["coordinates"].append(coord)
                guest["species"].append(pseudo_name)
                guest["site_data"].append(
                    [pseudo_name, element, coord, charge]
                )

                raspa_site_params = {}
                missing_charge_pseudos = []

                for pseudo_name, meta in pseudo_atoms_meta.items():
                    element = str(meta["element"]).strip()
                    mass = float(meta.get("mass"))

                    if "charge" in meta:
                        charge = float(meta["charge"])
                    else:
                        charge = 0.0
                        missing_charge_pseudos.append(pseudo_name)

                    lj = lj_params.get(pseudo_name, None)

                    raspa_site_params[pseudo_name] = [
                        pseudo_name,
                        element,
                        mass,
                        charge,
            ...
            if missing_charge_pseudos:
                unique = sorted(set(missing_charge_pseudos))
                logger.warning(
                    "Some charges were not found in force_field.json.\n"
                    "\t\t These sites were assigned a default charge of 0.0.\n"
                    "\t\t As a result, DL POLY will compute binding energies strictly using\n"
                    "\t\t Lennard-Jones interactions for these atoms."
                )
        ...
        coords_all = []
        species_all = []
        site_data_unfiltered_all = []
        site_data_grouped_all = []
        guest_dicts = []

        for guest in guests:

            valid_labels_all.append(guest["valid_labels"])
            coords_all.append(guest["coordinates"])
            species_all.append(guest["species"])
            site_data_unfiltered_all.append(guest["site_data"])

            grouped = {}
            for pseudo_name, element, coord, charge in guest["site_data"]:
                key = (pseudo_name, element)
                if key not in grouped:
                    grouped[key] = {
                        "coords": [],
                        "charges": [],
                    }
                grouped[key]["coords"].append(coord)
                grouped[key]["charges"].append(charge)

            grouped_list = []
            for (pseudo_name, element), data in grouped.items():
                grouped_list.append(
                    [
                        pseudo_name,
                        element,
            ...
            site_data_grouped_all.append(grouped_list)

            guest_dicts.append(
                {
                    "valid_labels": guest["valid_labels"],
                    "coordinates": guest["coordinates"],
                    "species": guest["species"],
                    "site_data": grouped_list,
                    "site_data_unfilted": guest["site_data"],
                }
            )
        ...
        self.gala.guest_field = selected_guests
        self.gala.raspa_lj_params = raspa_site_params
        return selected_guests, guests_reduced

    
    def _epsilon_K_to_kcal(self, epsilon_K: float) -> float:
        KB_KCAL_PER_MOL_K = 0.00198720425864083
        return float(epsilon_K) * KB_KCAL_PER_MOL_K
...
class GuestMolecule(Molecule):
    ...
    def _generate_raspa_field(self) -> None:
        """Generate a RASPA FIELD file based on gala data extracted from RASPA inputs earlier."""
        directory = self.gala.directory
        field_path = os.path.join(directory, "FIELD")

        raspa_lj_params = getattr(self.gala, "raspa_lj_params", None)
        guest_field = getattr(self.gala, "guest_field", None)
        raspa_guests = getattr(self.gala, "raspa_guests", None)
        
        if raspa_lj_params is None:
            raise RuntimeError("gala.raspa_lj_params is missing. Ensure parse_raspa ran first.")
        if guest_field is None:
            raise RuntimeError("gala.guest_field is missing. Ensure parse_raspa populated it first.")

        try:
            n_guests = len(guest_field[0])
        except Exception as exc:
            raise RuntimeError("Could not infer number of guest components from guest_field[0].") from exc

        if n_guests <= 0:
            raise RuntimeError("No guest components detected in guest_field.")

        guest_names = []
        try:
            for i in range(n_guests):
                species = guest_field[2][i]
                guest_names.append(str(species[0]) if species else f"guest{i+1}")
        except Exception:
            guest_names = [f"guest{i+1}" for i in range(n_guests)]

        guest_site_data_blocks = []
        guest_pseudos_all = set()

        for i in range(n_guests):
            try:
                guest_site_data = guest_field[3][i]
            except Exception as exc:
                raise RuntimeError(
                    f"Could not extract guest site_data_unfiltered for component {i}. "
                    "Expected guest_field[3][i] to exist."
                ) from exc

            if not isinstance(guest_site_data, (list, tuple)) or len(guest_site_data) == 0:
                raise RuntimeError(f"Guest component {i} has empty site data in guest_field[3][{i}].")

            guest_site_data_blocks.append(guest_site_data)

            for row in guest_site_data:
                pseudo = str(row[0]).strip()
                guest_pseudos_all.add(pseudo)

        framework_structure = getattr(self, "get_cube_structure", None)
        if framework_structure is None:
            framework_structure = getattr(self, "structure_with_sites", None)
        if framework_structure is None:
            raise RuntimeError("No framework structure found (get_cube_structure / structure_with_sites missing).")

        framework_sites = getattr(framework_structure, "sites", None)
        if framework_sites is None:
            framework_sites = framework_structure

        framework_atom_count = len(framework_sites)

        fold_factor = getattr(self.gala, "gcmc_grid_factor", (1, 1, 1))
        try:
            framework_nummols = int(fold_factor[0]) * int(fold_factor[1]) * int(fold_factor[2])
        except Exception:
            framework_nummols = 1
            logger.warning("fold_factor was not usable, defaulting Framework NUMMOLS to 1.")

        framework_param_values = []
        for pseudo_name, vals in raspa_lj_params.items():
            if str(pseudo_name).strip() in guest_pseudos_all:
                continue
            framework_param_values.append(vals)

        if len(framework_param_values) < framework_atom_count:
            logger.warning(
                "Framework parameter list from raspa_lj_params is shorter than number of framework atoms. "
                "This likely means your ordering assumption is broken."
            )

        def _mix_lb(lj_a, lj_b):
            if not lj_a or not lj_b:
                return (0.0, 0.0)
            eps_a, sig_a = lj_a
            eps_b, sig_b = lj_b
            eps_a = float(eps_a)
            ...
            if eps_a <= 0.0 or eps_b <= 0.0 or sig_a <= 0.0 or sig_b <= 0.0:
                return (0.0, 0.0)

            return (np.sqrt(eps_a * eps_b), 0.5 * (sig_a + sig_b))

        title = getattr(self, "structure_name", None) or "RASPA_FIELD"

        with open(field_path, "w") as out:
            out.write(f"{title}\n")
            out.write("UNITS kcal\n")
            out.write(f"molecular types {n_guests + 1}\n")

            # Guest block(s)
            for idx, i in enumerate(range(n_guests)):
                guest_site_data = guest_site_data_blocks[i]

                out.write(f"&guest {raspa_guests[idx]}\n")
                out.write("NUMMOLS 0\n")
                out.write(f"ATOMS {len(guest_site_data)}\n")

                for pseudo, element, coord, charge in guest_site_data:
                    pseudo = str(pseudo).strip()

                    if pseudo not in raspa_lj_params:
                        raise KeyError(f"Guest pseudo atom '{pseudo}' not found in gala.raspa_lj_params")

                    mass = float(raspa_lj_params[pseudo][2])
                    x, y, z = float(coord[0]), float(coord[1]), float(coord[2])
                    q = float(charge)

                    out.write(f"{pseudo:<8s}{mass:12.6f}{q:12.6f}{x:12.6f}{y:12.6f}{z:12.6f}\n")

                n_atoms = len(guest_site_data)
                out.write("rigid 1\n")
                out.write(f"{n_atoms} " + " ".join(str(k) for k in range(1, n_atoms + 1)) + "\n")
                out.write("finish\n")

            # Framework block
            out.write("Framework\n")
            out.write(f"NUMMOLS {framework_nummols}\n")
            out.write(f"ATOMS {framework_atom_count}\n")

            for site, params in zip(framework_sites, framework_param_values):
                element = str(params[1]).strip()
                mass = float(params[2])
                charge = float(params[3])
                out.write(f"{element:<8s}{mass:12.6f}{charge:20.12f}{1:8d}{1:8d}\n")

            out.write("finish\n")

            # VDW block
            guest_labels = sorted(guest_pseudos_all)
            framework_labels = sorted(set(str(s.species_string).strip() for s in framework_sites))
            all_labels = framework_labels + guest_labels

            label_to_lj = {}
            for g in guest_labels:
                lj = raspa_lj_params.get(g, None)
                label_to_lj[g] = lj[4] if (lj and len(lj) > 4) else None

            for elem in framework_labels:
                chosen = None
                for pseudo_name, vals in raspa_lj_params.items():
                    if str(pseudo_name).strip() in guest_pseudos_all:
                        continue
                    v_elem = str(vals[1]).strip()
                    v_lj = vals[4] if len(vals) > 4 else None
                    if v_elem == elem and v_lj is not None:
                        chosen = v_lj
                        break
                label_to_lj[elem] = chosen

            pairs = []
            for i, a in enumerate(all_labels):
                for b in all_labels[i:]:
                    eps, sig = _mix_lb(label_to_lj.get(a), label_to_lj.get(b))
                    pairs.append((a, b, eps, sig))

            N_TRAILING_ZEROS = 2
            zeros = " ".join(["0.0"] * N_TRAILING_ZEROS)

            out.write(f"VDW {len(pairs)}\n")
            for a, b, eps, sig in pairs:
                out.write(f"{a:<8s}{b:<8s} lj {eps:12.6f} {sig:12.6f} {zeros}\n")

            out.write("close\n")

        logger.info("Generated RASPA FIELD at %s", field_path)
    ...
    def Make_Files(self):
        ...
        if len(binding_sites) == 0:
            logger.info(
                "GALP process concluded successfully - no binding sites were identified."
            )
            sys.exit(0)

        if self.gala.method == "RASPA":
            if not getattr(self.gala, "_field_generated", False):
                self._generate_raspa_field()
                self.gala._field_generated = True
    ...
    def Submit_DL_Poly(
        ...
        if no_submit:
            logger.info("GALP input files generated; skipping job submission\n")
            return
    ...
    def gala_postproc(self, filepath, cell_data):
        ...
        for directories in os.listdir():
            if directories.startswith(f"{guest}_bs"):
                ...
                with open(
                    os.path.join(
                        self.gala.output_directory, "%s_galp_binding_sites.xyz" % guest
                    ),
                    "w",
                ) as gala_out:
...
class GuestSites:
    ...
    def maxima(self):
        ...
        """
        if self.gala.method == "FASTMC" or self.gala.method == "RASPA":
    ...
    def load_cube_data(self):
        ...
        if self.gala.gen_ml_plot:
            if os.path.exists(probability_file):
                    logger.error(
                        "A probability plot already exists in the present directory. "
                        "Please remove the probability plot before continuing."
                    )
                    raise RuntimeError("Probability plot already exists")
            
            from pymatgen.io.ase import AseAtomsAdaptor
            try:
                from ase.io import read as ase_read
            except ImportError as exc:
                logger.error("ASE not found, cannot use ML prediction")
                raise exc
            ...
            try:
                ...
                logger.error("DeepAPD package not found, cannot use ML prediction")
                raise exc
            ...
            if guests == "O":
                if self.gala.ml_pressure == 1.0:
                    ...
                    logger.error(
                        "Unsupported pressure for ML prediction with guest CH4. "
                        "Supported pressures are 1 bar and 65 bar."
                    )
                    raise ValueError("Unsupported ML pressure for CH4")
            elif guests == "Xe":
                if self.gala.ml_pressure == 1.0:
                    ml_guest = "Xe_1bar"
                else:
                    logger.error(
                        "Unsupported pressure for ML prediction with guest Xe. "
                        "Supported pressure is 1 bar."
                    )
                    raise ValueError("Unsupported ML pressure for Xe")
            else:
                logger.error("Guest selection not supported for ML prediction! \n" + 
                        "The following are supported: \n" +
                        "\t\tXe @ 1 bar" + 
                        "\n\t\tCH4 @ 1 bar" + 
                        "\n\t\tCH4 @ 65 bar")
                raise ValueError("Unsupported guest for ML prediction")
            
            logger.info("Using ML guest configuration: %s", ml_guest)
            ...
            if len(_cifs) == 0:
                logger.error(
                    "No CIF files found in the working directory. "
                    "A single CIF file is required to use ML prediction."
                )
                raise FileNotFoundError("No CIF file found for ML prediction")
            if len(_cifs) > 1:
                logger.error(
                    "More than one CIF file found in the working directory. "
                    "Exactly one CIF file is required to use ML prediction."
                )
                raise ValueError("Multiple CIF files found for ML prediction")
        ...
        if os.path.exists(probability_file):
            ...
            try:
                ...
                self.get_entropy(localdata, dir, probability_file)
            ...
            try:
                ...
                self.get_entropy(site_data, dir, probability_file_unfolded)
    ...
    def get_entropy(self, volumetric_data, dir, file):
        ...
        f = open(os.path.join(dir, "entropy.txt"), "a")
        f.write(
            f"{file.split('/')[-1]}\n\tEntropy: {S:.4f}\n"
        )
        f.close()
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        filename=f"{gala_input.directory}/galp.log",
        filemode="w",
    )
    logger = logging.getLogger(__name__)

    logger.info("Reading Input File")
    logger.info(f"Running GALP at: {gala_input.directory}")
    ...
    if os.path.exists(
        os.path.join(gala_input.directory, "DL_POLY.output")):
        os.remove(
            os.path.join(gala_input.directory, "DL_POLY.output"))
    if os.path.exists(
        os.path.join(gala_input.directory, "entropy.txt")):
        os.remove(
            os.path.join(gala_input.directory, "entropy.txt"))
    if os.path.exists(
        os.path.join(gala_input.directory, "galp.err")):
        os.remove(
            os.path.join(gala_input.directory, "galp.err"))
    else:
        pass
